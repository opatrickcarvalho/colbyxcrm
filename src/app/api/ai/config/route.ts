...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, handoff_policy, is_active, auto_reply_enabled, auto_reply_max_per_conversation, handoff_agent_id, api_key, embeddings_api_key',
      )
  ...
  try {
    ...
    const systemPrompt =
      typeof body.system_prompt === 'string' && body.system_prompt.trim()
        ...
        : null
    const handoffPolicy =
      typeof body.handoff_policy === 'string' && body.handoff_policy.trim()
        ? body.handoff_policy.trim()
        : null
