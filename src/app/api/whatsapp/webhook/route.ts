...
async function processMessage(
  ...
  // Everything that happens *after* the message is stored — reopen,
  // broadcast reply flag, Flows, automations, AI auto-reply,
  // message.received — is shared with the UAZAPI webhook. It lived inline
  // here until the second provider arrived, which meant none of it ran for
  // UAZAPI.
  await runInboundSideEffects({
    ...
    // Snapshot taken before this message landed — see the re-open note
    // in runInboundSideEffects.
    conversationStatus: conversation.status,
    providerMessageId: message.id,
    contentType,
    contentText: contentText ?? message.text?.body ?? '',
    interactiveReplyId,
