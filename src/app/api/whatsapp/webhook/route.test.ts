...
const h = vi.hoisted(() => ({
  ...
  state: {
    ...
    /** Fork-only: the suspended-account gate this webhook runs first. */
  ...
  createClient: () => ({
    from(table: string) {
      switch (table) {
        ...
        case 'accounts':
          // Fork-only: this webhook checks for a suspended account before
          // doing any inbound work (migration 040). Upstream has no such
          // query, so its harness never modelled the table and every test
          // in this file died here on `unexpected table: accounts`.
          return {
            select: () => ({
              eq: () => ({
                maybeSingle: () =>
                  Promise.resolve({
                    data: { status: h.state.accountStatus },
                    error: null,
                  }),
              }),
            }),
