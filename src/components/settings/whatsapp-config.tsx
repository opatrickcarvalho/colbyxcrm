...
export function WhatsAppConfig() {
  ...
  /**
   * What the operator picked this session. `null` means "not chosen
   * yet", so the saved row decides — that way the screen opens on the
   * provider that is actually connected.
   */
  ...
  // The picker is the whole point of the two-provider work: an account
  // runs on exactly one backend at a time (whatsapp_config is
  // UNIQUE(account_id)), and this is where the operator chooses which.
  //
  // The saved row wins over local state so a reload lands on whatever
  // is actually connected rather than the default.
  const activeProvider: 'meta' | 'uazapi' =
    providerChoice ?? (config?.provider === 'uazapi' ? 'uazapi' : 'meta');
  ...
  const providerPicker = (
    <Card className="mb-6">
      <CardHeader>
        ...
        <CardDescription>
          Choose how this account connects to WhatsApp. Only one can be active
          at a time.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 sm:grid-cols-2">
        ...
        >
          <span className="block font-medium">Official Meta API</span>
          <span className="text-muted-foreground mt-1 block text-sm">
            Approved templates, 24-hour session window. Needs a Meta Business
        ...
        <button
          type="button"
          onClick={() => setProviderChoice('uazapi')}
          className={`rounded-lg border p-4 text-left transition ${
        ...
        >
          <span className="block font-medium">UAZAPI (QR code)</span>
          <span className="text-muted-foreground mt-1 block text-sm">
            Unofficial. Scan a QR code to link a number — no templates, no
            session window.
          </span>
        </button>
      </CardContent>
        ...
        <UazapiConnect
          initialStatus={
            config?.provider === 'uazapi'
              ? (config.status as 'connected' | 'connecting' | 'disconnected')
              : 'disconnected'
          }
          onChanged={() => {
