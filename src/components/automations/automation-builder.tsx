...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  async function save() {
    ...
    try {
      const payload = {
        name: state.name || t('untitledName'),
...
function StepRenderer({
  ...
  // controls out of reach. Nested cards fill the column they were
  // given instead.
  //
...
function ConditionBranches({
  ...
  return (
    ...
    // a branch is a fraction of the screen, and a viewport-based
    // sm:grid-cols-2 split it anyway, leaving two columns too narrow
    // to render a step in.
