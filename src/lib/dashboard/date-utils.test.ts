...
describe('mondayIndex', () => {
  /**
   * Build a LOCAL calendar date.
   *
   * `new Date("2026-05-18")` looks equivalent but is not: a date-only
   * string is parsed as midnight *UTC*, while `mondayIndex` reads
   * `getDay()`, which is local. West of Greenwich those disagree — in
   * UTC-3 that instant is Sunday 21:00 — so these assertions only held
   * on machines at or ahead of UTC. The multi-argument constructor is
   * local in both halves, which is what makes the test mean what it
   * says. (Month is 0-indexed: 4 = May.)
   *
   * Local is the right frame here, not a workaround: both production
   * callers in dashboard/queries.ts read local weekdays, because the
   * dashboard shows the operator's week rather than UTC's.
   */
  const localDate = (year: number, month0: number, day: number) =>
    new Date(year, month0, day);

  it('maps Monday → 0 and Sunday → 6', () => {
    expect(mondayIndex(localDate(2026, 4, 18))).toBe(0); // Mon
    expect(mondayIndex(localDate(2026, 4, 19))).toBe(1); // Tue
    expect(mondayIndex(localDate(2026, 4, 23))).toBe(5); // Sat
    expect(mondayIndex(localDate(2026, 4, 24))).toBe(6); // Sun
  });

  it('aligns with DOW_SHORT_MON_FIRST labels', () => {
    expect(DOW_SHORT_MON_FIRST[mondayIndex(localDate(2026, 4, 18))]).toBe(
      'Mon'
    );
    expect(DOW_SHORT_MON_FIRST[mondayIndex(localDate(2026, 4, 24))]).toBe(
      'Sun'
    );
  });
});
